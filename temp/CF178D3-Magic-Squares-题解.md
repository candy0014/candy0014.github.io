...
47 1#include <bits/stdc++.h>2using namespace std;3int n;4int du[200005],vis[200005];5// du数组存每个点度数，vis数组为 1/0 代表此节点 有/无 被删去 6int head[200005],tot;7struct EDGE{int nex,to;}s[400005];8void add(int u,int v){9    s[++tot].nex=head[u],s[tot].to=v;10    head[u]=tot;11    du[u]++;12}13vector<int>ans;14void dfs(int u,int fa){15    vis[u]=1,ans.push_back(u);16    for(int i=head[u];i;i=s[i].nex){17        int v=s[i].to;18        if(v==fa||vis[v]) continue;//已经删掉的子节点不需要递归 19        dfs(v,u);20    }21}22void work(int u,int fa){23    for(int i=head[u];i;i=s[i].nex){24        int v=s[i].to;25        if(v==fa) continue;26        work(v,u);27    }28    if(du[u]%2==0){29        dfs(u,fa);//删光以u节点为根的子树 30        du[fa]--;//由于以u节点为根的子树已经处理完了，所以只需改变u父亲的度数即可 31    }32}33int main(){34    scanf("%d",&n);35    for(int i=1,u;i<=n;i++){36        scanf("%d",&u);37        if(u) add(i,u),add(u,i);38    }39    work(1,0);40    if(!vis[1]){ 41        printf("NO\n");42        return 0;43    }44    printf("YES\n");//若1号节点被删除了，其余肯定都被删除了 45    for(int i=0;i<n;i++) printf("%d\n",ans[i]);46    return 0;47}cpp
